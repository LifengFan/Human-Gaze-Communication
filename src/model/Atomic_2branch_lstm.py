...
class Atomic_2branch_lstm(torch.nn.Module):
    def __init__(self, args):
        super(Atomic_2branch_lstm, self).__init__()
        ...
        for pass_rnd in range(iterN):
            for b_id in range(N):
                ...
                if pass_rnd == (iterN - 1):
                    ...
                    en_out = self.en_fc1(torch.cat((h_n.clone().view(-1), edge_out[b_id,...].view(-1)),0))  # 40+48*2
