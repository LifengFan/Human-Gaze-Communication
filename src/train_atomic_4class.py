...
def main(args):
    ...
    train_set, validate_set, test_set, train_loader, validate_loader, test_loader = get_data.get_data_atomic_4(args)

    #model = models.Atomic(args)
    model=models.Atomic_2branch_4class(args)
...
#--------------------------------------------------
    ...
    get_metric_from_confmat(confmat, '4class')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, atomic_label_batch,_) in enumerate(train_loader):
...
def validate(validate_loader, model, criterion, epoch, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, atomic_label_batch,_) in enumerate(validate_loader):
...
def test(test_loader, model, criterion, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, atomic_label_batch,_) in enumerate(test_loader):
...
def parse_arguments():
    ...
    project_name = 'train_atomic_2branch_4class'
    ...
    parser.add_argument('--batch-size', type=int, default=2, help='Input batch size for training (default: 10)')
