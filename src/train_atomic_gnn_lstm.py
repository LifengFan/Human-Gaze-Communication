...
def main(args):
    ...
    model = models.Atomic_GNN_lstm(args)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (node_batch, pos_batch, attmat_batch, atomic_label_batch) in enumerate(train_loader):
        # sl_gt [N, 10, max_node_num]

        batch_size=node_batch.shape[0]
        #assert batch_size==args.batch_size, 'wrong batch size!{} {}'.format(batch_size, args.batch_size)

        optimizer.zero_grad()

        if args.cuda:
            nodes = (torch.autograd.Variable(node_batch)).cuda()
        ...
        with torch.set_grad_enabled(True):
            ...
            #head_patch_sq, pos_sq, attmat_sq, atomic_label
            pred_atomic= model(nodes,poses,attmat_gt)

            train_loss=0
            for bid in range(batch_size):

                # todo: check pred_atomic dim [N, 6]
...
def validate(validate_loader, model, criterion, epoch, args):
    ...
    for i, (node_batch, pos_batch, attmat_batch, atomic_label_batch) in enumerate(validate_loader):
...
def test(test_loader, model, criterion, args):
    ...
    for i, (node_batch, pos_batch, attmat_batch, atomic_label_batch) in enumerate(test_loader):
        ...
        with torch.set_grad_enabled(False):
            ...
            pred_atomic = model(nodes,poses,attmat_gt)#[N, 6, 1,1,1]
...
def parse_arguments():
    ...
    project_name = 'train_atomic_gnn_lstm_iter1'
