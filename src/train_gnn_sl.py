...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (patches, poses, sl_gt, num_rec, attmat_gt) in enumerate(train_loader):
        ...
        with torch.set_grad_enabled(True):
            ...
            sl_pred, attmat_pred= model(patches, poses, num_rec, attmat_gt)
            # sl_pred [N, 6 , 7] attmat_pred [N, 6, 6]
            ...
            for bid in range(batch_size):
                ...
                for nid in range(valid_node_num):
                    tmp_loss=criterion[0](sl_pred[bid, nid, :].unsqueeze(0), sl_gt[bid, nid].unsqueeze(0)) \
                             + criterion[1](attmat_pred[bid,nid,:], attmat_gt[bid,nid,:])
                    ...
                    pred = torch.argmax(sl_pred[bid, nid, :], dim=-1)
...
def parse_arguments():
    ...
    parser.add_argument('--lr', type=float, default=0.01, help='Initial learning rate (default : 1e-3)')
