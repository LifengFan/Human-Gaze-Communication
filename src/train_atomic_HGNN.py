...
def main(args):
    ...
    model_args = {'roi_feature_size': args.roi_feature_size, 'edge_feature_size':12,
    ...
    model = models.Atomic_HGNN(model_args, args)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (head_batch, pos_batch, attmat_batch, atomic_label_batch) in enumerate(train_loader):
        ...
        with torch.set_grad_enabled(True):
            ...
            pred_atomic= model(heads, poses, attmat_gt, args)
            ...
            if i %10 ==0:
                print('Epoch: {}/{} Iter: {} Training Loss: {:.4f} Total Avg Acc: {:.4f} Learning rate {}'.format(epoch, args.epochs - 1, i,
                                                                                            train_loss.item(),total_acc.avg, args.lr))                      
            # save tmp checkpoint
            if i % 90 == 0:
                utils.save_checkpoint({
                    'epoch': epoch + 1,
